...
// Keep original camera filenames when requested while still allowing a photo
// to have a useful description and a deliberate position in the carousel.
const photoMetadata = new Map([
  ['IMG_2749.jpg', { alt: 'Amber Richardson presenting a research poster', position: 'last' }],
])
...
export async function getLabPhotos() {
  ...
  const names = entries
    ...
    .sort((left, right) => {
      const leftIsLast = photoMetadata.get(left)?.position === 'last'
      const rightIsLast = photoMetadata.get(right)?.position === 'last'
  ...
  return photos.map((name) => ({
    ...
    alt: photoMetadata.get(name)?.alt ?? describePhoto(name),
  }))
}
